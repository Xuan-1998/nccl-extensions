...
:class:`AllocConfig`::
    ...
    cfg = AllocConfig(alloc_fn=alloc_addr, free_fn=free_addr)

Lifetime requirement
--------------------

The trampoline owns the underlying machine-code stub. ``my_alloc`` and
``my_free`` MUST stay alive for at least as long as the resulting group; if
either is garbage-collected while NCCL EP still holds the pointer, the next
call from C lands in freed memory and crashes. Stash them at module scope or
on an object that outlives the group.
